...
- 从 Windows 开始菜单打开 **「ESP-IDF 6.0 PowerShell」**（版本号可能不同），它会自动配置好所有环境变量。

> 当工具链装在**非默认位置**（不在 `C:\Users\你的用户名\.espressif`）时，需要手动配置环境变量，见文末「附录：特殊环境配置」。
